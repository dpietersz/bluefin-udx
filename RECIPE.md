...
| `/etc/fonts/conf.d/05-bluefin-writable-cache.conf` | Registers `/var/cache/fontconfig` as the first (writable, shared) fontconfig cachedir | Color-emoji/serif-tofu fix (system-cache half). Under composefs `/usr/share/fonts` has an epoch checksum; an fc-cache run before it materialises writes a valid-but-EMPTY cache that validates forever, hiding real serif fonts so Chromium/Electron pick a Nerd Font for `serif`. A writable shared cachedir lets `fc-cache-boot.service` (below) publish a *verified* system cache. Because it's the FIRST cachedir it must never hold a poisoned cache — hence the guard in the service. |
...
| `/usr/lib/systemd/system/fc-cache-boot.service` (enabled) | Guarded `fc-cache --force` into `/var/cache/fontconfig`, `Before=display-manager.service` | Rebuilds the system caches early, then VERIFIES a canary font (`Noto Serif`) is visible and DELETES the cache if the scan came up empty — so it can never publish a poisoned valid-empty cache (the failure the old `--system-only` variant caused; `--system-only` also exits 1 on this image). A missing cache safely triggers a live rescan later; a poisoned one never heals. The authoritative repair is the user-scope `fontconfig-heal.sh` in dotfiles. Enabled via the `systemd` module in `common.yml`. |

## Build-time maintenance steps

| Step | Purpose | Why baked |
|---|---|---|
| `files/scripts/rebuild-font-cache.sh` | Rebuild system fontconfig cache after font-bearing RPMs/apps are layered | Partial emoji/serif-tofu mitigation. **Not sufficient on its own** — rechunk normalizes font-dir mtimes *after* this cache is built, so at runtime fontconfig deems the baked cache stale and rescans `/usr/share/fonts` live. The durable fix has two halves: the guarded `fc-cache-boot.service` above (system cache, can't poison) plus the host-side dotfiles verify-and-heal (`refresh-fontconfig-cache.service` → `fontconfig-heal.sh`, which rebuilds `~/.cache` until real serif fonts are visible). Keep this bake as defense-in-depth for fresh boots; don't treat it as the whole fix. |
