...
| `/etc/fonts/conf.d/05-bluefin-writable-cache.conf` | Registers `/var/cache/fontconfig` as the first (writable, shared) fontconfig cachedir | Durable color-emoji/tofu fix. Under composefs, `/usr/share/fonts` has an epoch mtime that never matches any baked cache, so fontconfig rescans it live on every process start; that rescan races login font-dir churn and leaves Helium/Teams in monospace + tofu for the session. A writable shared cachedir lets `fc-cache-boot.service` (below) publish a valid system cache all sessions read. |
...
| `/usr/lib/systemd/system/fc-cache-boot.service` (enabled) | `fc-cache --system-only --force` into `/var/cache/fontconfig`, `Before=display-manager.service` | Rebuilds the system caches early, in a quiet namespace before the DM and any distrobox/user font churn, capturing the stable epoch mtime so the cache validates for the session. This is the runtime half that makes the fix durable. Enabled via the `systemd` module in `common.yml`. |

## Build-time maintenance steps

| Step | Purpose | Why baked |
|---|---|---|
| `files/scripts/rebuild-font-cache.sh` | Rebuild system fontconfig cache after font-bearing RPMs/apps are layered | Partial COLRv1 emoji-tofu mitigation. **Not sufficient on its own** — rechunk normalizes font-dir mtimes *after* this cache is built, so at runtime fontconfig deems the baked cache stale and rescans `/usr/share/fonts` live. The durable fix is the runtime `fc-cache-boot.service` above (writable shared `/var/cache/fontconfig`, rebuilt before the DM) plus the host-side dotfiles user cache (`dot_config/fontconfig/conf.d/75-emoji-color.conf` + `refresh-fontconfig-cache.service`). Keep this bake as defense-in-depth for fresh boots; don't treat it as the whole fix. |
